...
| ファイル | 内容 | 言語 |
|---|---|---|
| `README.md` / `README.ja.md` | エンドユーザー向けの使い方 | 日本語/英語 |
...
| `docs/p2p_detailed_design.md` | P2P 詳細設計 | 英語 |
| `docs/addictive_ideas.md` | 中毒性向上アイデア集 | 英語 |
| `docs/implementation_roadmap.md` | 実装ロードマップ詳細 | 英語 |
...
- **ゲームロジックはWASMに移行する**。`curion.rs` / `player.rs` / `generator.rs` / `synthesis.rs` はUI依存を持たせない
- **UIはターミナル風のPWA**になる。スマホでも「ターミナルっぽい見た目」で遊べるWebアプリ
- **タブUIはそのまま活かせる**。TUI のタブ構成をそのままWeb UIのタブに対応させる設計にしておく
- **各タブの情報構成・レイアウトもPWAと同一**。TUIで決めた「何をどのタブに出すか」がそのままスマホ画面の設計になる。TUI段階で情報設計を固める
- **`ui.rs` と `plain.rs` はターミナル専用**。PWA移行時はWebフロントエンドが代替する
- **ロジック層はWASM非対応のcrateを使わない**こと。`nostr-sdk` / `tokio` はWASM対応済みだが、今後依存追加時は要確認
