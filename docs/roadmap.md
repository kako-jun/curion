...
- [x] 意味空間ベクトル装備効果 (#38) — Issue #39 の 16 次元 latent vector を 10 タグ (熱 / 速度 / 秩序 / 混沌 / 生命 / 機械 / 夢 / 暴力 / 幸運 / 純粋) の `SemanticProfile` に投影し、装備中 Curion の意味タグから `EquipmentEffect { xp_multiplier, san_decay_modifier, rare_probability_bonus, synthesis_success_bonus }` を導出。Phase 1 では `xp_multiplier` (Player::add_curion の XP に乗算) と `san_decay_modifier` (add_play_time の SAN 減衰に乗算) のみ実適用し、残り 2 つは UI 表示と計算のみ用意。`Player::equipment: EquipmentSlot` (curion_id Option) を `#[serde(default)]` で旧セーブ互換、装備 Curion が collection から消えた場合は `EquipmentEffect::none()` フォールバック。Collection 一覧で `e` キーを押すと選択中の Curion を装備し、Dashboard 概要に装備中 Curion とその 10 タグ強度（上位 3 個）+ 効果サマリを 2 行で表示
- [x] ロジック層を WASM 対応に整理 (#23) — `curion.rs` / `player.rs` / `generator.rs` / `synthesis.rs` / `achievement.rs` / `latent.rs` / `semantic.rs` / `equipment.rs` / `cooldown.rs` / `san.rs` / `daily_mission.rs` / `evolution.rs` の 12 ファイルを audit し、`std::fs` / `std::path` / `std::env` / `ratatui` / `crossterm` / `rustyline` / `nostr_sdk` / `tokio` / `dirs` の import が一切混入していないことを確認 (san.rs に "ratatui や Player への参照を持たない" というコメントが 1 件あるのみ)。外部データ参照は `include_str!` のみで WASM ターゲットでもビルド時埋め込みで動く。`docs/wasm_prep.md` を新規作成し、ロジック層と IO/UI 層の境界、grep ベースの監視手段、将来の `curion-core` / `curion-tui` crate 分割設計と移行手順を明文化。本 PR では crate 分割そのものは行わず、`src/` 内のファイル単位の境界維持にとどめる
